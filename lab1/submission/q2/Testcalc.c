...
void test_add()
{
	int8_t output = add(120, 40);
	TEST_ASSERT_EQUAL(160, output);
}
...
// Question 2 Part 1: Call a corresponding function in the
// test suite that will verify we are using signed integers

void test_negnum()
{
	TEST_ASSERT_TRUE(negnum(-14));
	TEST_ASSERT_TRUE(negnum(16));
	TEST_ASSERT_FALSE(negnum(0));
}
...
//gcc src/main.c src/calc.c -I./src -o main.exe
//gcc test/Testcalc.c src/calc.c unity/src/unity.c -I./src -I./unity/src -o testcalc.exe