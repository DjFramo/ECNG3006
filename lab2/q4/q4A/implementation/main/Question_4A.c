...
// Q4A - Equal Task Priority Testing with task monitoring using Logging
...
// Task priorities
const int TASK_ONE_PRIORITY = 1;
const int TASK_TWO_PRIORITY = 1;
const int TASK_THREE_PRIORITY = 1;
SemaphoreHandle_t Mutex = NULL;

// Log current mutex holder
static inline void log_mutex_holder(const char *tag, SemaphoreHandle_t mtx) {
    TaskHandle_t holder = xSemaphoreGetMutexHolder(mtx);
    if (holder) {
        ESP_LOGI(tag, "Mutex holder: %s", pcTaskGetName(holder));
    } else {
        ESP_LOGI(tag, "Mutex holder: none");
    }
}
...
void task_one(void *pvParameters) {
    while (1) {
        ESP_LOGI(TAG_TASK1, "Attempting to take Mutex...");
        log_mutex_holder(TAG_TASK1, Mutex);

        if (xSemaphoreTake(Mutex, portMAX_DELAY)) {
            ESP_LOGI(TAG_TASK1, "Successfully took Mutex");
            log_mutex_holder(TAG_TASK1, Mutex);

            gpio_set_level(LED_GPIO, LOW);  // Active-low LED ON

            if (xSemaphoreGive(Mutex) == pdTRUE) {
                ESP_LOGI(TAG_TASK1, "Successfully gave Mutex");
                log_mutex_holder(TAG_TASK1, Mutex);
            } else {
                ESP_LOGW(TAG_TASK1, "Failed to give Mutex");
            }
            ...
            ESP_LOGW(TAG_TASK1, "Failed to take Mutex");
        }
...
void task_two(void *pvParameters) {
    while (1) {
        ESP_LOGI(TAG_TASK2, "Attempting to take Mutex...");
        log_mutex_holder(TAG_TASK2, Mutex);

        if (xSemaphoreTake(Mutex, portMAX_DELAY)) {
            ESP_LOGI(TAG_TASK2, "Successfully took Mutex");
            log_mutex_holder(TAG_TASK2, Mutex);

            gpio_set_level(LED_GPIO, HIGH);  // Active-low LED OFF

            if (xSemaphoreGive(Mutex) == pdTRUE) {
                ESP_LOGI(TAG_TASK2, "Successfully gave Mutex");
                log_mutex_holder(TAG_TASK2, Mutex);
            } else {
                ESP_LOGW(TAG_TASK2, "Failed to give Mutex");
            }
            ...
            ESP_LOGW(TAG_TASK2, "Failed to take Mutex");
        }
    }
}
...
void app_main(void) {
    ...
    if (Mutex == NULL) {
        ...
        log_mutex_holder(TAG_MAIN, Mutex);
    }
